...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_SESSION['users'][$email])) {
        ...
        $register_success = true; // Set flag register success
    }
}
?>
...
<head>
  ...
  <title>Get Started - Finance Application</title>
...
<section class="bg-[#1e272b] text-[#cbd5e1] p-8 w-full max-w-md mx-auto mt-12 rounded-lg shadow-md fade-in delay-2">
  <h2 class="text-3xl text-center font-semibold mb-6">Get Started</h2>
  <p class="text-center text-lg mb-6">Create your free account and start managing your finances today.</p>

  <form action="get-started.php" method="POST" class="space-y-5">
    <div>
      <label for="name" class="block text-sm mb-1">Full Name</label>
      <input type="text" id="name" name="name" required class="w-full px-4 py-2 rounded bg-[#263238] text-white hover-input" />
    </div>
    <div>
    ...
    <button type="submit" class="w-full py-2 mt-4 bg-blue-600 hover:bg-blue-700 rounded text-white font-medium hover-btn">Create Account</button>
  </form>
...
<!-- Notifikasi -->
<div id="registerNotification" class="notification">
...
<script>
  <?php if (isset($register_success) && $register_success): ?>
    document.getElementById('registerNotification').style.display = 'block';  // Show notification immediately
    setTimeout(function() {
      document.getElementById('registerNotification').style.display = 'none';  // Hide notification after 3 seconds
